...
dfData$submitted[grepl("P", dfData$id, fixed = TRUE)] <- "Papier"
dfData$submitted[grepl("Z", dfData$id, fixed = TRUE)] <- "Zeitung"
...
dfData$t_desc_od  <- dfData$t_desc  %>% str_replace(., "Drone brood removal & ", "") %>% str_trim()
dfData$t_short_od <- dfData$t_short %>% str_replace(., "Drone & ", "") %>% str_trim()

dfData$c_desc_od  <- dfData$c_desc %>%
  str_replace(., "SPRING Drone brood removal & ", "") %>%
  str_trim()
dfData$c_desc_od  <- dfData$c_desc_od %>%
  str_replace(., "SUMMER Drone brood removal & ", "") %>%
  str_trim()
dfData$c_desc_od  <- dfData$c_desc_od %>%
  str_replace(., "WINTER Drone brood removal & ", "") %>%
  str_trim()
dfData$c_short_od <- dfData$c_short %>% str_replace(., "SP-Drone & ", "") %>% str_trim()
dfData$c_short_od <- dfData$c_short_od %>% str_replace(., "SU-Drone & ", "") %>% str_trim()
dfData$c_short_od <- dfData$c_short_od %>% str_replace(., "WI-Drone & ", "") %>% str_trim()
