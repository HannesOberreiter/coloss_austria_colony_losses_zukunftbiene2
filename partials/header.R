...
RAW <- bind_rows(RAW)
RAW <- RAW %>% select(-starts_with("...")) # Drop empty cols
rm(i, files)
...
RAW$hives_winter[ is.na( RAW$hives_winter )] <- 0
RAW$hives_lost[ is.na( RAW$hives_lost )] <- 0
RAW$lost_a[ is.na( RAW$lost_a )] <- 0 # Lost - Queen
RAW$lost_b[ is.na( RAW$lost_b )] <- 0 # Lost - Elements
RAW$lost_c[ is.na( RAW$lost_c )] <- 0 # Lost - Other
# Symptoms clean NAs
RAW$symp_a[ is.na( RAW$symp_a )] <- 0
RAW$symp_b[ is.na( RAW$symp_b )] <- 0
RAW$symp_c[ is.na( RAW$symp_c )] <- 0
RAW$symp_d[ is.na( RAW$symp_d )] <- 0
RAW$symp_e[ is.na( RAW$symp_e )] <- 0
# Sum the symptoms
RAW <- RAW %>% 
  rowwise() %>% 
  mutate(
    symp_total = sum(across(starts_with("symp_")))
  )
...
RAW <- RAW %>% 
  mutate(
    ...
    # Values for Queens
    hives_spring_queen <- hives_winter - lost_a,
    # Loss rate per company %
    lost_rate <- hives_lost / hives_winter * 100,
    lost_rate_e <- hives_lost_e / hives_winter * 100,
    # hives per apiary
    hives_per_apiary <- hives_winter / apiaries
  )

## Submitting Type ---------------------------------------------------------
RAW$submitted <- "Internet"
RAW$submitted[grepl("P", RAW$id, fixed = TRUE)] <- 'Papier'
RAW$submitted[grepl("Z", RAW$id, fixed = TRUE)] <- 'Zeitung'
...
rm(colnameyn, i, rsum10, rsum12, treatmentexp10, vlogical, treatmentexp12, x10, x12)
